...
## sequence database
my $NCBI_DIR = "/usr/local/bin/";
my $SEQ_DB = "/root/data/uniref90";
...
my $DATA_DIR = abs_path(join '/', $dir,"data/disopred"); # the path of the data directory
$ENV{DSO_LIB_PATH} = join '/', abs_path($dir), "data/dso_lib/"; # the path of the library directory used by the nearest neighbour classifier
...
my $args = join ' ', $NCBI_DIR."psiblast", "-db", $SEQ_DB, "-query", $fasta_fn, "-inclusion_ethresh", "0.001", "-out_pssm", $chk_file, "-num_iterations", "3", "-num_alignments", "0";
